import React from 'react'

export default function ContactDetail() {
    return (
        <div className="six-types">

            <div className='footer-section'>
                <h2>GreenTripo</h2>
                <p>
                    Green Tripo is a global travel platform
                    offering affordable flight and hotel
                    bookings, delivering seamless,
                    reliable, and personalized travel
                    experiences for leisure and business
                    travelers worldwide.
                </p>
                {/* <div> 
                {/* <div className='images-detail'>
                        <img className='fb' src='data:image/png;base64,'></img>
                    </div> */}
                {/* <div>
                        <img className='twitter' style={{}} src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSjT7nZyVxKmO0OGOyp7yuZDN7Y67A9olZVWg&s'></img>
                    </div> */}
                {/* <img className='instagram' style={{}} src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSf-0c7nWX4Jo9neuLUlIWcUutbzUBuKRgnyQ&s'></img>
                    <img className='linked-in' style={{}} src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQJO45msUOXwAm2ajZTDLznzkec0csKNMpQEA&s'></img>
                </div> */}
            </div>


            <div className="footer-section">
                <h3> Quick Links</h3>
                <hr className='hr-container' />
                <ul>
                    <li> Home</li>
                    <li>About Us</li>
                    <li> Services</li>
                    <li>Best Deals</li>
                    <li>Business Class</li>
                    <li> Destinations</li>
                    <li>FAQs</li>
                    <li>Contact Us</li>
                </ul>
            </div>

            <div className="footer-section">
                <h3> Useful Links</h3>
                <hr className='hr-container' />
                <ul>
                    <li>  Cookie Policy</li>
                    <li> Data Protection Policy</li>
                    <li> Disclaimer</li>
                    <li> Refund Policy </li>
                    <li> Terms & Conditions</li>
                    <li>   Sitemap</li>
                </ul>
            </div>

            <div className="footer-section">
                <h3>Airlines</h3>
                <hr className='hr-container' />
                <ul>

                    <li>Air France </li>
                    <li>Alaska Airlines</li>
                    <li>American Airlines </li>
                    <li> Delta Airlines</li>
                    <li> Frontier Airlines</li>
                    <li> JetBlue Airways</li>
                    <li> KLM Airlines</li>
                    <li> Southwest Airlines</li>
                    <li> Spirit Airlines</li>
                    <li> United Airlines</li>
                </ul>
            </div>

            <div className="footer-section">
                <h3> Get In Touch</h3>
                <hr className='hr-container' />
                <ul>
                    <li> Phone: +1-805-702-1258 (U.S.A.) </li>
                    <li> Email: customercare@greentripo.com </li>
                    <li> Incorporation Number: 469105</li>
                    <li>Business Registration Certificate: 53855321</li>
                    <li>  D-U-N-S Number: 66-339-1625</li>
                </ul>
            </div>
        </div>
    )
}
